...
export function createDateInTimezone(
  ...
  console.log(
    `Creating date in timezone: ${timezone} with date: ${dateString} and time: ${timeString}`,
  );
  ...
  try {
    ...
    console.log(
      `Parsed components: year=${year}, month=${month}, day=${day}, hour=${hour}, minute=${minute}`,
    );
    ...
    if (timezone.includes(" ")) {
      ...
      if (parts.length > 1) {
        ...
        console.log(
          `Extracted clean timezone: ${cleanTimezone} from ${timezone}`,
        );
      }
    }

    // IMPORTANT: We need to properly create a date in the specified timezone
    // First create the date string with the timezone
    ...
    // Use the Intl.DateTimeFormat to convert to the ISO string with timezone info
    ...
    // Get formatted version in the target timezone
    const formattedDate = formatter.format(localDate);
    console.log(`Formatted date in ${cleanTimezone}: ${formattedDate}`);

    // Then create a UTC date for comparison
    ...
    if (futureDate <= now) {
      ...
      console.warn(
        `Created date is not in the future. Now: ${now.toISOString()}, Created: ${futureDate.toISOString()}`,
      );
      ...
      const isToday =
        today.getFullYear() === scheduleDay.getFullYear() &&
        today.getMonth() === scheduleDay.getMonth() &&
        today.getDate() === scheduleDay.getDate();

      if (isToday) {
        console.log(
          `Scheduled time is for today but already passed. Will publish immediately.`,
        );
