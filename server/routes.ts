...
export async function registerRoutes(app: Express): Promise<void> {
  ...
  apiRouter.post("/projects/save", async (req: Request, res: Response) => {
    try {
      const { name, description, formData } = req.body;
      
      if (!name || !formData) {
        return res.status(400).json({ error: "Project name and form data are required" });
      }
      
      // Get store ID from connection
      const connection = await storage.getShopifyConnection();
      if (!connection) {
  ...
  apiRouter.get("/projects", async (req: Request, res: Response) => {
    try {
      ...
      const projects = await storage.getSavedProjects(connection.id);
  ...
  apiRouter.get("/projects/:id", async (req: Request, res: Response) => {
    try {
      ...
      const project = await storage.getSavedProject(projectId);
  ...
  apiRouter.delete("/projects/:id", async (req: Request, res: Response) => {
    try {
      const projectId = parseInt(req.params.id);
      await storage.deleteSavedProject(projectId);
