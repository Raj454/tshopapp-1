...
router.get('/callback', shopifyAuthOptional, async (req: Request, res: Response) => {
  try {
    ...
    try {
      const shopInfoResponse = await fetch(`https://${shopDomain}/admin/api/2025-07/shop.json`, {
        headers: {
      ...
      const blogsResponse = await fetch(`https://${shopDomain}/admin/api/2025-07/blogs.json`, {
        headers: {
          'X-Shopify-Access-Token': access_token,
        },
      });
