...
let promptText = `Generate a well-structured, SEO-optimized blog post with the EXACT title "${request.topic}" in a ${toneStyle} tone, ${contentLength}. ${copywriterPersona}${mediaContext}${audienceContext}${keywordContext}
    ...
    TABLE OF CONTENTS REQUIREMENTS:
    - NEVER EVER create a Table of Contents manually in HTML
    - NEVER write <h3>Table of Contents</h3> or <ol> or <li> tags for TOC
    - ONLY add this exact marker at the very beginning: <!-- TABLE_OF_CONTENTS_PLACEMENT -->
    - Do NOT create any TOC HTML - the system handles this automatically
    ...
    - FORBIDDEN: Manual TOC creation, <ol> lists for navigation, anchor links in content body
    - The automatic TOC will be generated by the system and will work perfectly
