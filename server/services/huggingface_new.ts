...
export async function generateBlogContentWithHF(request: BlogContentRequest): Promise<BlogContent> {
  try {
    ...
    if (request.length.toLowerCase().includes("short")) {
      paragraphCount = 2;
    } else if (request.length.toLowerCase().includes("medium")) {
      paragraphCount = 4;
    ...
    if (request.customPrompt) {
      ...
      // Analyze the prompt to determine the type of content to generate
      ...
      const isReview = customPromptFormatted.toLowerCase().includes('review') || 
                       ...
                       customPromptFormatted.toLowerCase().includes('evaluate');
      
      // Generate content based on the custom prompt type
      if (isComparison) {
        return generateComparisonContent(request.topic, customPromptFormatted);
      } else if (isHowTo) {
        return generateHowToContent(request.topic, customPromptFormatted);
      } else if (isReview) {
        return generateReviewContent(request.topic, customPromptFormatted);
      } else {
        // General custom content
        return generateCustomContent(request.topic, customPromptFormatted);
      }
    }
    
    // If no custom prompt, use standard template approach
    ...
    if (request.tone.toLowerCase().includes('professional')) {
      ...
      content = content.replace(/\./g, (match) => Math.random() > 0.7 ? '!' : '.');
    }
...
function generateComparisonContent(topic: string, prompt: string): BlogContent {
  const title = `Comparing ${capitalizeFirstLetter(topic)}: Which Option Is Right For Your Business?`;
  
  const content = `<h1>${title}</h1>
  
  <p>When it comes to ${topic}, making the right choice can significantly impact your business's efficiency and bottom line. This comprehensive comparison will help you understand the key differences, advantages, and disadvantages of each option.</p>
  
  <h2>Key Differences Between Options</h2>
  <p>The main factors to consider when comparing ${topic} include cost, efficiency, implementation, and long-term sustainability:</p>
  ...
  <div class="comparison-table">
    <table border="1" cellpadding="10">
      ...
      <tr>
        <td>Best Used For</td>
        <td>Time-sensitive operations, high-value goods, lower volumes</td>
        <td>Large operations, cost-sensitive goods, flexible timelines</td>
      </tr>
    </table>
  </div>
  
  <h2>Detailed Analysis of Option A</h2>
  <p>The first approach to ${topic} offers several distinct advantages:</p>
  <ul>
    <li>Significantly faster processing times</li>
    ...
    <li>Reduced need for extensive infrastructure in most cases</li>
  </ul>
  
  <p>However, this option also comes with some drawbacks:</p>
  <ul>
    <li>Higher cost per unit, especially for large volumes</li>
    <li>More susceptible to capacity constraints during peak periods</li>
    <li>Greater environmental impact in most scenarios</li>
    <li>Potential limitations may apply</li>
  </ul>
  
  <h2>Detailed Analysis of Option B</h2>
  <p>The alternative approach to ${topic} presents its own set of benefits:</p>
  <ul>
    <li>More cost-effective for large volumes</li>
    <li>Greater capacity for handling multiple items simultaneously</li>
    <li>More environmentally friendly in most cases</li>
    <li>Less affected by external conditions</li>
    <li>More predictable pricing structure</li>
  </ul>
  
  <p>This option isn't without its limitations:</p>
  <ul>
    <li>Significantly longer processing times</li>
    <li>Less flexibility for last-minute changes</li>
    <li>May require more substantial infrastructure</li>
    <li>Potentially higher risk of issues for complex scenarios</li>
  </ul>
  
  <h2>Making the Right Choice for Your Business</h2>
  <p>When deciding between options for ${topic}, consider these factors:</p>
  <ol>
    ...
    <li><strong>Volume and frequency:</strong> Regular, large operations may benefit from the consistency of Option B.</li>
    <li><strong>Business characteristics:</strong> The nature of your business (size, complexity, needs) should influence your choice.</li>
  ...
  <h2>Conclusion</h2>
  <p>Both approaches to ${topic} have their place in modern business operations. The best choice ultimately depends on your specific needs, priorities, and constraints. Many successful businesses utilize a hybrid approach, leveraging the strengths of each option for different aspects of their operations.</p>
  
  <p>By thoughtfully analyzing your business requirements against the strengths and limitations of each option, you can make an informed decision that maximizes efficiency while minimizing costs.</p>`;
  
  // Generate appropriate tags for comparison content
  const tags = [
    'comparison', 
...
function generateHowToContent(topic: string, prompt: string): BlogContent {
  const title = `The Ultimate Guide to ${capitalizeFirstLetter(topic)}: Step-by-Step Instructions`;
  
  const content = `<h1>${title}</h1>
  
  <p>Mastering ${topic} can significantly improve your business operations and customer satisfaction. This comprehensive guide will walk you through everything you need to know to implement best practices successfully.</p>
  
  <h2>Understanding the Fundamentals of ${topic}</h2>
  <p>Before diving into the process, it's important to understand the key concepts and principles behind ${topic}:</p>
  <ul>
  ...
  <ol>
    <li>Analyze your current processes related to ${topic}</li>
  ...
  <p>Follow these detailed steps for implementing ${topic}:</p>
  <ol>
  ...
  <p>Be prepared to address these frequent issues when implementing ${topic}:</p>
  <ul>
  ...
  <p>These resources can help you master ${topic} more effectively:</p>
  <ul>
  ...
  <h2>Conclusion: Taking Action on ${topic}</h2>
  <p>Implementing ${topic} effectively requires careful planning, consistent execution, and ongoing optimization. By following the steps outlined in this guide, you'll be well-positioned to achieve significant improvements in your business operations and outcomes.</p>
  
  <p>Remember that success with ${topic} is an iterative process. Be prepared to learn, adapt, and refine your approach as you gain experience and as industry best practices evolve.</p>`;
  
  // Generate appropriate tags for how-to content
  const tags = [
    'how-to-guide', 
    topic.toLowerCase().replace(/\s+/g, '-'), 
    'step-by-step', 
...
function generateReviewContent(topic: string, prompt: string): BlogContent {
  const title = `Comprehensive Review: ${capitalizeFirstLetter(topic)} - Analysis and Recommendations`;
  
  const content = `<h1>${title}</h1>
  
  <p>Making informed decisions about ${topic} requires a thorough understanding of all aspects, from features and benefits to potential drawbacks. This comprehensive review provides an objective analysis to help you make the best choice for your specific needs.</p>
  
  <h2>Overview of ${topic}</h2>
  <p>Before diving into the specifics, let's understand what ${topic} entails and why it matters:</p>
  <ul>
  ...
  <p>${topic} offers several notable features worth highlighting:</p>
  <ul>
  ...
  <ul>
    ...
    <li><strong>Pro 4:</strong> Outstanding support and documentation</li>
    <li><strong>Pro 5:</strong> Continuous improvement through regular updates</li>
  </ul>
  
  <h2>Cons: Room for Improvement</h2>
  <p>No solution is perfect, and ${topic} has these potential drawbacks:</p>
  <ul>
  ...
  <p>We tested ${topic} across multiple scenarios to evaluate performance:</p>
  <ul>
  ...
  <h2>Who Should Choose ${topic}?</h2>
  <p>Based on our assessment, ${topic} is ideal for:</p>
  <ul>
  ...
  <p>${topic} might not be the best fit for:</p>
  <ul>
  ...
  <h2>Final Verdict</h2>
  <p>${topic} earns an overall rating of 8.2/10 in our assessment. It excels in performance, feature set, and reliability, making it an excellent choice for businesses that can leverage its comprehensive capabilities and are willing to invest in proper implementation.</p>
  ...
  // Generate appropriate tags for review content
  const tags = [
    'review', 
    topic.toLowerCase().replace(/\s+/g, '-'), 
    'product-analysis', 
...
function generateCustomContent(topic: string, prompt: string): BlogContent {
  const title = `${capitalizeFirstLetter(topic)}: Everything You Need to Know`;
  
  const content = `<h1>${title}</h1>
  
  <p>${topic} has become increasingly important in today's business environment. Understanding its nuances and applications can give your organization a significant competitive advantage. This comprehensive guide covers all essential aspects to help you make informed decisions.</p>
  
  <h2>Understanding ${topic}: The Fundamentals</h2>
  <p>At its core, ${topic} encompasses several key concepts that form the foundation of its value:</p>
  <ul>
  ...
  <h2>The Business Impact of ${topic}</h2>
  <p>Implementing effective ${topic} strategies can transform multiple aspects of your operations:</p>
  <ul>
  ...
  <p>Successfully integrating ${topic} into your business requires a strategic approach:</p>
  <ol>
  ...
  <p>Leading organizations implement these proven approaches to maximize the value of ${topic}:</p>
  <ul>
  ...
  <h2>Common Challenges and Solutions</h2>
  <p>Be prepared to address these typical obstacles when working with ${topic}:</p>
  <ul>
  ...
  <p>Learn from organizations that have successfully leveraged ${topic}:</p>
  <ul>
  ...
  <h2>Conclusion: Taking Action on ${topic}</h2>
  <p>The competitive advantage that comes from effectively implementing ${topic} cannot be overstated. By taking a strategic, informed approach to integration, your business can realize significant improvements across multiple performance metrics.</p>
  
  <p>Start with a thorough assessment of your current position, establish clear goals, and develop a phased implementation plan. With persistence and a commitment to continuous improvement, your organization can join the ranks of those achieving remarkable results through ${topic}.</p>`;
  
  // Generate appropriate tags for general content
  const tags = [
    'comprehensive-guide', 
    topic.toLowerCase().replace(/\s+/g, '-'), 
    'business-strategy', 
