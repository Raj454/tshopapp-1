...
export class ShopifyService {
  ...
  private legacyConnection: ShopifyConnection | null = null;

  /**
   * Set connection for legacy compatibility
   * @param connection The Shopify connection
   */
  public setConnection(connection: ShopifyConnection): void {
    this.legacyConnection = connection;
  }

  /**
   * Get blogs using legacy connection (for backward compatibility)
   * @returns Array of blogs
   */
  public async getBlogsLegacy(): Promise<any[]> {
    if (!this.legacyConnection) {
      throw new Error('No legacy connection set');
    }

    // Convert legacy connection to store format
    const store: ShopifyStore = {
      id: this.legacyConnection.id,
      shopName: this.legacyConnection.storeName,
      accessToken: this.legacyConnection.accessToken,
      scope: '',
      defaultBlogId: this.legacyConnection.defaultBlogId || null,
      isConnected: this.legacyConnection.isConnected || true,
      lastSynced: this.legacyConnection.lastSynced,
      installedAt: new Date(),
      uninstalledAt: null,
      planName: null,
      chargeId: null,
    ...
    return await this.getBlogs(store);
  }

  /**
   * Test connection using legacy connection
   */
