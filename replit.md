...
- **Claude API Performance Optimizations (August 14, 2025)**: Updated to latest Claude Sonnet 4.0 model, reduced max tokens from 8000 to 5000 for faster generation, added 60-second timeout to prevent hanging requests, and enabled automatic meta optimization that was previously disabled. Fixed null metaTitle issue by setting fallback values during initial blog post creation.
