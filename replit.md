...
- **Secondary Images Deduplication Fix**: Enhanced the duplicate detection logic in secondary images processing to check both ID and URL matches, preventing the same image from appearing multiple times in the Selected Secondary Images tab. Improved debugging logs for better tracking of duplicate removal.
