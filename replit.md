...
```
Changelog:
- July 7, 2025. CRITICAL SCHEDULING TIMEZONE ISSUE COMPLETELY FIXED: Resolved the root cause of immediate publishing instead of proper scheduling by implementing accurate timezone conversion from store timezone (EDT) to UTC storage - fixed createDateInTimezone function to properly add UTC offset (EDT +4 hours) ensuring scheduled posts wait for correct future time - verified with test post showing user input "17:00 EDT" correctly converts to "21:00 UTC" for proper 7.5-hour future scheduling - all scheduled posts now respect store timezone settings and publish at intended times
