...
- July 31, 2025. UNIVERSAL DYNAMIC DATAFORSEO SYSTEM COMPLETED: Completely eliminated ALL remaining static category terms from DataForSEO service to create truly universal keyword generation system - removed hardcoded category assumptions from extractGenericTerms() and generateBroadCategoryTerms() methods, replaced with dynamic product category extraction strategies that work for any store type (fashion, electronics, home goods, etc.) - lowered search volume threshold from 100 to 50 monthly searches and enhanced fallback trigger to activate when fewer than 5 keywords found - updated comprehensive documentation showing DataForSEO endpoints (/v3/keywords_data/google/search_volume/live and /v3/dataforseo_labs/google/keyword_suggestions/live) and universal extraction strategies - system now dynamically extracts categories from any product name structure without hardcoded assumptions, ensuring the app works universally for any type of Shopify store
